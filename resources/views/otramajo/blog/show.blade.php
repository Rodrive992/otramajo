...
@section('content')
    <style>
        ...
        /* Sin altura fija: se adapta al alto real de la imagen */
        .photo-frame {
            margin-bottom: 2rem;
        }
        ...
        .photo-frame-inline img {
            width: 100%;
            height: auto;
            /* La altura la define la proporción de la imagen */
            object-fit: contain;
            /* No recorta la imagen; la ajusta completa */
            display: block;
        ...
        @media (max-width: 768px) {
            ...
            /* Sin alturas fijas también en móvil */
            .photo-frame {
                margin-bottom: 2rem;
            }

            .photo-frame-inline {
                /* solo mantenemos márgenes si hace falta */
            }
                ...
                <div class="grid lg:grid-cols-3 gap-12 items-start">
                    ...
                    <div class="lg:col-span-2">
                        <div class="text-content section-fade-in">
                            ...
                            @if (session('ok'))
                                <div
                                    class="mb-4 rounded-lg bg-green-50 border border-green-200 text-green-800 px-4 py-3 text-sm">
                                    {{ session('ok') }}
                                </div>
                            @endif
                            ...
                            <article class="article-body prose max-w-none">
                                ...
                                @if ($articulo->imagen_secundaria2)
                                    @php
                                        ...
                                        $isUrl2 =
                                            $img2 &&
                                            \Illuminate\Support\Str::startsWith($img2, ['http://', 'https://']);
                                ...
                                @if ($articulo->imagen_secundaria3)
                                    @php
                                        ...
                                        $isUrl3 =
                                            $img3 &&
                                            \Illuminate\Support\Str::startsWith($img3, ['http://', 'https://']);
                            ...
                            <div class="mt-10 border-t border-[color:var(--om-gris-claro)] pt-8">
                                ...
                                @if ($articulo->comentarios->isNotEmpty())
                                    ...
                                    @foreach ($articulo->comentarios as $coment)
                                        <div
                                            class="mb-6 border border-[color:var(--om-gris-claro)] rounded-xl p-4 bg-white shadow-sm">
                                ...
                                <div class="mt-6">
                                    ...
                                    <form action="{{ route('blog.comentar', $articulo->id) }}" method="POST"
                                        class="space-y-4">
                                        @csrf

                                        <div>
                                            <label class="om-label">Nombre</label>
                                            <input type="text" name="nombre_usuario" class="om-input" required
                                                value="{{ old('nombre_usuario') }}">
                                        </div>

                                        <div>
                                            <label class="om-label">Correo electrónico</label>
                                            <input type="email" name="correo_electronico" class="om-input" required
                                                value="{{ old('correo_electronico') }}">
                                        </div>

                                        <div>
                                            <label class="om-label">Comentario</label>
                                            <textarea name="comentario" rows="4" class="om-textarea" required>{{ old('comentario') }}</textarea>
                                        </div>
                    ...
                    <div class="lg:col-span-1">
                        <div class="photo-frame sticky top-24">
                            <img src="{{ $src }}" alt="Portada {{ $articulo->titulo }}"
                        ...
                        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach ($relacionados as $r)
                                @php
                                    ...
                                    $rurl =
                                        $rimg && \Illuminate\Support\Str::startsWith($rimg, ['http://', 'https://'])
                                            ? $rimg
                                            : ($rimg
                                                ? asset('storage/' . $rimg)
